...
//     - send_message：目标为存续规划子代理时放行（问答续流/转交续轮），
//       否则需计划已批准；
...
export function apply(ctx, config) {
  ...
  const diagPath = typeof cfg.diagFile === 'string' && cfg.diagFile !== '' ? cfg.diagFile : join(process.env.DSH_HOME !== undefined && process.env.DSH_HOME !== '' ? process.env.DSH_HOME : '.dsh', 'extra-plan-request-errors.jsonl')
  ...
  ctx.on('tools/pre-execute', (exec, next) => {
    ...
    if (name === 'pwsh' && pwshMutationMatches(exec)) {
      if (!escape && state.route !== 'direct' && state.approved !== true) {
        return { kind: 'deny', reason: '路由未确认/计划未批准：pwsh 只读探查可随时进行；创建/修改/删除文件的命令须先完成路由确认（选「直接执行」）或计划批准。' }
      }
      return next()
    }
    ...
    if (name === 'send_message') {
      ...
      if (!escape && state.approved !== true && !plannerIds.includes(target)) {
        return { kind: 'deny', reason: 'send_message 未放行：只能向存续中的规划子代理转达用户反馈（问答续流/转交续轮）；向其他子代理发消息需计划已批准。' }
      }
      return next()
    }
    return next()
