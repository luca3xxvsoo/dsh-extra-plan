...
//       子代理）起的 tool/call（除 save_plan）≥ exploreBudget 后拒绝后续
...
//   dd/install/rsync/scp/wget -O/curl -o/tar/zip/unzip/nano/vim/vi/truncate/fallocate/
//   shred/mount/umount/mkfs/包管理器(apt/yum/brew/npm install -g/pip install)/docker/kubectl。
const BASH_MUTATION = /\b(rm|mv|cp|mkdir|rmdir|touch|tee|chmod|chown|ln)\b(?=\s|$|\/)|git\s+(add|commit|checkout|switch|restore|clean|rm|mv|reset)\b|sed\s+(?:--in-place\b|(?:-[A-Za-z]*\s+)*-i\b)|(?:[0-9]?>>?(?!&\d)|&>|>&(?!\d))/i
...
// pwsh 命令文本（对象/字符串双形状，v11 修复）。
function pwshCommandOf(exec) {
...
function pwshMutationMatches(exec) {
  const cmd = pwshCommandOf(exec)
  return cmd !== '' && PWSH_MUTATION.test(cmd)
}

// bash 命令文本（对象/字符串双形状，与 pwshCommandOf 对等）。已探查核实：bash 与
// pwsh 的 exec.arguments 形状一致——dsh-tool-bash 与 dsh-tool-pwsh 的 defineTool 参数
// 定义均为 { command: { type: "string", required: true } }。
...
function bashMutationMatches(exec) {
  const cmd = bashCommandOf(exec)
  return cmd !== '' && BASH_MUTATION.test(cmd)
}
...
function deriveFlowState(events) {
  ...
  for (let i = lastHuman + 1; i < events.length; i += 1) {
    ...
    if (e.type === 'tool/call' && e.data !== null && typeof e.data === 'object' &&
      ...
      if (labels !== null) asks.set(e.data.callId, askKindOf(labels))
      continue
    }
...
function withBudgetNotice(message, notice) {
  ...
  const block = message.content[0]
  if (block === null || typeof block !== 'object' || block.type !== 'text' || typeof block.text !== 'string') return message
  if (block.text.indexOf(notice) !== -1) return message
  return { ...message, content: [{ type: 'text', text: block.text + '\n\n' + notice }] }
}
...
function budgetReminderText(remaining, budget, threshold) {
  ...
  return `本轮探查预算还剩 ${remaining} 次，请收紧探查、规划收尾，未查项记入待确认假设清单`
}
...
function budgetExhaustedReason(used, budget) {
  return `探查预算已耗尽（本轮已用 ${used}/${budget}）：停止进一步探查，基于已知信息产出规划方案与验收标准清单（含待确认假设清单）并调用 save_plan 落盘。方案须逐条标注【已探查核实】/【未探查·待确认】，未探查部分只能写为待确认假设、不得编造具体内容；用户下一条消息（意见转达/疑问答复）到达时探查预算将重置，阻塞性问题可停轮提出。`
}
...
export function apply(ctx, config) {
  ...
  const exploreBudget = Number.isInteger(cfg.exploreBudget) && cfg.exploreBudget > 0 ? cfg.exploreBudget : 12
  ...
  let ledgerFsPromise = null
  const usageCursors = new Map()
  let usageCursorsLoaded = false
  function ledgerFs() {
    if (ledgerFsPromise === null) ledgerFsPromise = import('node:fs').then((m) => m.default ?? m).catch(() => null)
    return ledgerFsPromise
  }
  async function foldUsage(agent, role) {
    ...
    try {
      const fs = await ledgerFs()
      if (fs === null || typeof fs.appendFileSync !== 'function') return
      if (!usageCursorsLoaded) {
        usageCursorsLoaded = true
        try {
          const saved = JSON.parse(fs.readFileSync(ledgerCursorPath, 'utf8'))
      ...
      fs.appendFileSync(ledgerPath, rows.join('\n') + '\n', 'utf8')
      const persisted = {}
      for (const [k, v] of usageCursors) {
        persisted[k] = v !== null && typeof v === 'object' ? { seq: v.seq, index: v.index } : v
      }
      try { fs.writeFileSync(ledgerCursorPath, JSON.stringify(persisted), 'utf8') } catch (error) { /* cursor 持久化尽力而为 */ }
    } catch (error) {
      if (!ledgerWarned) {
        ledgerWarned = true
  ...
  // 规划子代理：子会话且模型为 pro（tool-subagent-plan 行 agentOptions 指定）。
  ...
  function defineSavePlan() {
    return {
      name: 'save_plan',
      description: '把规划方案与验收标准清单落盘为工作区 .extra-plan 目录下的两个 Markdown 文件（原子双写：要么两个文件都写成、要么都不写）。plan 与 checklist 均为必填，缺一不可。taskName 为可选任务短名（用于文件名，插件自动净化非法字符）。plan 中每条步骤须标注探查状态【已探查核实】或【未探查·待确认】：未探查部分只能写为待确认假设，不得编造文件内容、数值、行号或文案。返回两个文件的路径。',
      parameters: {
        type: 'object',
        properties: {
  ...
  function registerSavePlan(agent) {
    if (savePlanRegistered.has(agent)) return
    savePlanRegistered.add(agent)
    const tools = agent.ctx !== undefined && agent.ctx !== null ? agent.ctx.get('tools') : undefined
    if (tools === undefined || typeof tools.register !== 'function') {
      console.warn('extra-plan: tools service unavailable — save_plan not registered')
      return
    }
    try {
      tools.register(defineSavePlan())
    } catch (error) {
      console.error(`extra-plan: save_plan registration failed: ${error instanceof Error ? error.message : String(error)}`)
    }
  }
  ...
  function registerSaveProbe(agent) {
    if (saveProbeRegistered.has(agent)) return
    saveProbeRegistered.add(agent)
    const tools = agent.ctx !== undefined && agent.ctx !== null ? agent.ctx.get('tools') : undefined
    if (tools === undefined || typeof tools.register !== 'function') {
      console.warn('extra-plan: tools service unavailable — save_probe not registered')
      return
    }
    try {
      tools.register(defineSaveProbe())
    } catch (error) {
      console.error(`extra-plan: save_probe registration failed: ${error instanceof Error ? error.message : String(error)}`)
    }
  }
  ...
  //    主会话注册 save_probe（recompose 不重发 session-start，pre-step 兜底）。
  ctx.on('agent/session-start', (payload) => {
    ...
    if (!isSubagentChild(agent)) registerSaveProbe(agent)
  })
  ...
  ctx.on('agent/pre-step', async (payload, next) => {
    if (payload.agent !== undefined) {
      ...
      if (!isSubagentChild(payload.agent)) registerSaveProbe(payload.agent)
    }
  ...
  const diagPath = typeof cfg.diagFile === 'string' && cfg.diagFile !== '' ? cfg.diagFile : join(process.env.DSH_HOME !== undefined && process.env.DSH_HOME !== '' ? process.env.DSH_HOME : join(homedir(), '.dsh'), 'extra-plan-request-errors.jsonl')
  ...
  // 4) 力度继承：子代理 agent/request 解析后注入父会话配置的 reasoningEffort。
  ctx.on('agent/request', async (payload, next) => {
    ...
    const effort = header !== undefined && header.config !== undefined && typeof header.config.reasoningEffort === 'string' ? header.config.reasoningEffort : ''
  ...
  ctx.on('tools/pre-execute', (exec, next) => {
    ...
    if (name === 'write' || name === 'edit') {
      if (!escape && state.route !== 'direct' && state.approved !== true) {
        return { kind: 'deny', reason: '路由未确认/计划未批准：动手改文件前必须先 ask_user_question 路由确认（选项固定为「直接执行」「进行pro规划」「不同意」），用户选择后才可动手。' }
      }
    ...
    if (isPwshMutation || isBashMutation) {
      ...
      if (!escape && state.route !== 'direct' && state.approved !== true) {
        return { kind: 'deny', reason: `路由未确认/计划未批准：${shellLabel} 只读探查可随时进行；创建/修改/删除文件的命令须先完成路由确认（选「直接执行」）或计划批准。` }
      }
    ...
    if (name === planToolName) {
      if (!escape && (state.route !== 'plan' || state.clarified !== true)) {
        return { kind: 'deny', reason: '规划子代理未放行：先完成路由确认（选「进行pro规划」）与意图澄清问答，再调用 subagent_plan。' }
      }
    ...
    if (name === 'save_probe') {
      if (!escape && (state.route !== 'plan' || state.clarified !== true)) {
        return { kind: 'deny', reason: '探查线索未放行：先完成路由确认（选「进行pro规划」）与意图澄清问答，再调用 save_probe 落盘探查线索。' }
      }
      return next()
    }
    if (name === 'subagent' || name === 'subagent_fork' || name === 'workflow' || name === 'ralph' || name === 'subagent_review') {
      if (!escape && state.approved !== true) {
        return { kind: 'deny', reason: '计划未批准：执行类委派须等用户对方案点「同意执行」；方案迭代请向存续规划子代理 send_message。' }
      }
