...
function budgetNoticeText(budget) {
  return `本轮探查预算上限为 ${budget} 次工具调用。注意：save_plan 有机械闸门 — 若方案含【未探查·待确认】或「待确认假设清单」将被拒绝，请确保所有步骤均已探查核实后再调用。`
}
...
function budgetExhaustedReason(used, budget) {
  return `探查预算已耗尽（本轮已用 ${used}/${budget}）：停止进一步探查。如存在有待确认项目/阻塞性问题，不调用 save_plan，直接在当轮输出中列出「待确认假设清单」/「阻塞性问题」并说明需要追加预算继续探查；如不存在待确认项目（全部已核实），基于已知信息产出规划方案与验收标准清单，正常调用 save_plan 落盘。`
}
    ...
    return {
      ...
      async execute(args, exec) {
        ...
        if (/【未探查·待确认】/.test(plan) || /待确认假设清单/.test(plan)) {
          throw new Error('save_plan: 方案中包含【未探查·待确认】步骤或「待确认假设清单」。请先申请追加预算继续探查，确认所有项均已探查核实（标注【已探查核实】）后再调用 save_plan。不要重试 save_plan — 先完成探查。')
        }
  ...
  ctx.on('tools/pre-execute', (exec, next) => {
    ...
    if (name === 'job_output') {
      ...
      if (args !== undefined && args !== null && typeof args === 'object' && args.wait === true) {
        return { kind: 'deny', reason: 'job_output 禁止带 wait: true 前台等待。请省略 wait 参数或设 wait: false，用 job_output 分次收集结果（job 完成后会收到通知）。' }
      }
      return next()
    }
    return next()
