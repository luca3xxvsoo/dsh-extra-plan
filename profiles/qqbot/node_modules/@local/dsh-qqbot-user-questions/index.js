...
export function apply(ctx, config) {
  ...
  bot.use(async (mwCtx, next) => {
    ...
    // ── /优先对话：把排队第一条提前交给 agent（静默，不回执）──
    const raw = msg.content ?? ''
    if (typeof raw === 'string' && raw.startsWith('/优先对话')) {
      // 群聊防御：mentionGate(passthrough=false) 已把未@bot 的群消息挡在链外，
      // 此处读 state.mention.wasMentioned 仅作二次防御（正常流程到不了）。
      ...
      // ── 新增：对齐 web store-steer，仅 agent 正在运行时才允许插队；空闲时静默无效 ──
