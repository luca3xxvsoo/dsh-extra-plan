...
⑩ 启用 pro 规划子代理（subagent_plan 工具，continuable 固定后台）：
   ...
   - **run_code 多调用容错硬闸门**：planner 用 run_code 批量只读探查时，≥2 个 tools.* 调用必须各自 try/catch 或 allSettled 或 .catch（否则组判定拒绝，教学式文案含「已保护 M 个」）；job_output 全角色禁 wait:true——等完成通知唤醒续轮，勿前台等待
   - **探查者委派**：one-shot 探查者 owner=委派者，planner 轮次结束会被宿主级联取消（owner disposed）——探查者未完成前不要结束本轮；已知引擎限制与修复候选见 ai-机制设计.md 教训索引「探查者级联中止」
