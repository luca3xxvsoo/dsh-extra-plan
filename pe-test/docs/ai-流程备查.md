...
- **前端**：`lib/client.js` 注册到 Plugins 页 `plugins.item`（`ctx.configForms.whileServed([NS], ...)` 包裹，`inject = ['slots','locale','configForms']`）；组件收 `{view, t, form}`——8 项用 `ownerProps.form.state` 播种、保存时 `form.mutate(ops, revision)`；2 项自绘控件走上面的 PUT。
