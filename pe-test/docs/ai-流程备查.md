...
- **前端**：`lib/client.js` 注册到 Plugins 页包详情页（位置在「包含的组件」上方）的 keyed 插槽 `plugins.bundle.config`（key = 包名 `@local/dsh-extra-plan`；`ctx.configForms.whileServed([NS], ...)` 包裹，`inject = ['slots','locale','configForms']`）；组件收 `{view, t, form}`——`form` 由宿主 `formFor(pkgName)` 按同一 settings 命名空间注入（形状 `{state, mutate}` 不变），8 项用 `form.state` 播种、保存时 `form.mutate(ops, revision)`；2 项自绘控件走上面的 PUT。
