...
⑩ 启用 pro 规划子代理（subagent_plan 工具，continuable 固定后台）：
   ...
   - **探查硬上限**（默认 18 次，计数含 save_plan）：每轮开局告知预算；剩余 ≤3 次注入一次「还剩 N 次」提醒（预算值 ≤3 时不注入；每轮只注入一次）；预算耗尽 → 拒绝后续工具调用并注入带数字收敛指令。预算自最近一条主会话发往子代理的消息起计：初始任务/续轮转达（kind=user/agent-message）= 一次重置 = 授权继续；插件自注入消息（生产者自有 kind，如预算提醒的 `plugin:@local/dsh-extra-plan`；**v4 行准入已禁旧包裹 `kind=plugin`**）不重置
...
- **2 项宿主行设置**（webFetch → 声明行 `plugins` 内 `tool-web` 行 `config.fetch`；toolPresentationMode → `tool-presentation` 行 `config.mode`）：2 项同样在 Config 内，与 8 项一并经客户端一次 mutate（10 op）写 settings 行；`PUT /api/dsh-extra-plan-settings/pro-config` 仅投影（body **仅**这 2 项，含其它键即 400）→ `ctx.configEditor.edit(presetEntry, (cur, inh) => restatePresetPlugins(cur, inh, { hostRowConfig }))` 整体重述 `config.plugins`；`GET` 只读 `configEditor.configuration()` 现值（声明行缺失时回落仓库模板出厂默认）。失败口径：投影失败 200+`projection.applied=false`（无 500 edit/reconcile 分流）；loopback 校验保留（非环回 403）。
