...
> 注册与兜底口径：注册成功后写「已注册」标记（WeakSet 按 agent 自去重）；分类 A（重名）与分类 B（永久性定义期错误）也写标记、记终态不再重试；只有服务未就绪与分类 C（可重试）不写标记，由后续 pre-step 重试。session-start 每会话只发一次且 recompose 不重发，故 pre-step 每步的幂等兜底是唯一重试通道（注册发生在 `await next()` 之前，与 decision.kind 无关）。pre-step 中注册的工具只进下一步 assembly（宿主先 assemble 快照、后派发 pre-step）。
