...
证据去向：③四层表的每一条都在「本仓库位置」列给出条目编号（宿主服务层 HS1-HS25、宿主组合与文件契约层 HK1-HK24、宿主数据与布局形状层 SD1-SD37、安装配置面 CF1-CF12），②覆盖总表的每一行给出该文件命中的条目编号或「无」。
...
| HS24 plugins/dsh-extra-plan/assets/presets/extra-plan/agent.cordis.yml L105-L122（config 键 9 个） | 预设行 config 经宿主 schema 校验后 1:1 传入 apply(ctx, config) | dsh-agent-presets 包内 lib/index.js | 把预设行 config 交给插件 apply（anchoredBootstrap、plannerModel、usageLedger 等） | 宿主改变行 config 传递方式或校验即插件配置读不到 | 【已核实】 |
...
| HK6 plugins/dsh-extra-plan/index.js L3175-L3176（agent/request） | agent/request（mode 为 waterfall；next() 返回 LlmCallConfig） | dsh-tool-cordis 包内 lib/index.js；派发点 dsh-agent-loop 包内 lib/index.js；类型 dsh-llm 包内 lib/types/call-config.d.ts | 规划者模型注入与力度继承（改写 provider 与 model 与 reasoningEffort 与 maxTokens） | 返回值形状或深冻结与克隆策略变化即改写无效或抛错 | 【已核实】 |
| HK7 plugins/dsh-extra-plan/index.js L3254-L3255（tools/pre-execute 硬闸门） | tools/pre-execute（mode 为 waterfall；exec 为 ToolExecution，next() 默认 allow） | dsh-tool-cordis 包内 lib/index.js；派发点 dsh-tools 包内 lib/index.js；类型 dsh-tools 包内 lib/types/index.d.ts | 四级机械闸门（route 与 purpose 与 clarified 与 approved）与多调用拆解拦截 | exec 契约或决策类型变化即闸门失效或误拦 | 【已核实】 |
...
| SD8 plugins/dsh-extra-plan/index.js L2518-L2521（subagent/descriptor 判定 continuable） | subagent/descriptor 事件（mode 取值 one-shot 或 continuable） | dsh-subagent 包内 lib/types/descriptor.d.ts | 区分可续轮子代理（规划者）与 one-shot（执行者与验收者） | mode 值域或事件版本变化即角色判定漂移 | 【已核实】 |
| SD9 plugins/dsh-extra-plan/index.js L820 与 L770（续轮转达判定；本行原第二处引用落 diff 增删区且与「续轮转达判定」语义不符，已读 HEAD 按语义改指 toolCallsSinceUser 的续轮转达锚点判定，原引用位置标【待确认】） | MessageSourceMap（plugin 与 tool 与 model）与续轮 source 的 kind 为 agent-message（form 为 relay） | dsh-llm 包内 lib/types/message.d.ts；dsh-subagent 包内 lib/types/continuation.d.ts | 识别续轮转达消息，避免把转达当新用户轮 | source 的 kind 或 form 改名即判定失效、预算被重复锚定 | 【已核实】 |
| SD10 plugins/dsh-extra-plan/index.js L187（session.snapshotEvents()） | Session.snapshotEvents(fromSeq, toSeqExclusive) | dsh-session 包内 lib/types/index.d.ts | 读取会话事件快照（已弃用 events getter） | 方法签名变化即读取失败 | 【已核实】 |
| SD11 plugins/dsh-extra-plan/index.js L2586-L2591（requestHeader 与 header.config） | Session.requestHeader() 返回 EpochHeader（内含 config 为 LlmCallConfig） | dsh-session 包内 lib/types/index.d.ts；dsh-llm 包内 lib/types/call-config.d.ts | 上溯父会话当前 provider 与 model 与 maxTokens 做继承 | 返回值形状或 config 位置变化即继承取不到值 | 【已核实】 |
| SD12 plugins/dsh-extra-plan/index.js L2693（agent.session.append 写 sandbox/mode） | Session.append(type, data) 与 sandbox/mode 事件负载（mode 与 source） | dsh-session 包内 lib/types/index.d.ts；dsh-sandbox-policy 包内 lib/types/session-mode.d.ts | 为委派子会话写沙箱下限（本插件唯一一次会话事件写入） | append 签名或事件键与负载变化即写入失败或被忽略 | 【已核实】 |
| SD13 plugins/dsh-extra-plan/index.js L243-L245（overrideOf 与 defaultMode） | sandboxPolicy（服务名）+ overrideOf(session) + defaultMode | dsh-sandbox-policy 包内 lib/index.js | 计算会话有效沙箱模式（overrideOf 内部走 sessionProjections 的 stateOf） | 服务方法签名或投影键变化即模式判定失效 | 【已核实】 |
...
| SD15 plugins/dsh-extra-plan/index.js L2600-L2605（llm.listModels(provider)） | llm（服务名）+ listModels(provider) 返回 LlmModelInfo 数组 | dsh-llm 包内 lib/index.js（签名目录 dsh-tool-cordis 包内 lib/index.js） | 设置页列出可选规划者模型 | 服务名或返回项形状变化即列表为空 | 【已核实】 |
