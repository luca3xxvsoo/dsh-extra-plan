...
| 预设自愈核对 | lib/preset-sync.js | 启动时 hash 比对下发；两条保留链：同 hash → idle（不读改写现场正文，用户改词逐字保留）；hash 变化 → 先复制新厂商模板、恢复 10 项 UI 设置、再整组迁移合法旧 gateWords（7 叶定点写回 + 共享 validator 复验；缺失/非法/歧义整组用新模板值，禁止部分迁移）；厂商模板 gateWords 在 hash/idle 判定前严格校验，坏模板抛错且目标目录逐字节不变；manifest format=2 + settingsMigration 10 项 + gateWordsMigration 7 项（只记状态不记词值） |
...
| PTC三维装配矩阵 | pe-test/tools/step-04-路由与写闸门.mjs | 实际执行 2×2×3×2×5=120 格，逐格断言 A/C/M/F-L/五角色、C7 0/7、catalog 0/2、普通 skill、HP 与 HN/HB 基线；同文件另含 P0-4 会话状态生命周期与末轮 usage 监听器级用例（e 段 P4-1~P4-24 期望不变 + 新增 P4-25+ 增量对拍：双 session 同 root 隔离、跨 session 锚点、disposed 清理、临时账本的 final flush/幂等/续载去重/cursor 降级、P4-24 provider/cw/rs 新字段落盘与五字段零行跳过） |
...
| 配置热读／生效标志（live-config，2026-09-23） | lib/live-config.js（createLiveConfig：7 项热读 getter + creativeMode 预留 getter）；index.js 7 个箭头 getter 与消费点 | 设置页改 YAML 后同会话即时生效（立即生效：runcodeCatchGate/plannerPromptSuffix/exploreBudget；新会话或新子代理生效：anchoredBootstrap/plannerModel/otherAgentModel/crossProviderPlannerModel；重启生效且未热读：creativeMode/webFetch/toolPresentationMode）。路径决议 configPath → DSH_EXTRA_PLAN_CONFIG_PATH → 默认路径（与 settings.js agentCordisPath 同公式，刻意不 import settings.js 以免把 schemastery 拉进 index.js 的 import 图）；构造期无条件读盘一次（以文件真值作首拍基准，堵住「新会话 apply 传入改前快照且 mtimeMs+size 未变」的边界缺口，2026-09-23 方案 A）；此后 mtimeMs+size 未变时零读盘零解析，变化才 captureSettings 全量解析；失败回退 apply 期快照 + console.warn 一次；无 per-Agent 缓存失效策略（新 agent = 新 WeakMap 键） |
