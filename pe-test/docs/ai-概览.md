...
| 功能域 | 位置 | 备注 |
|:--|:--|:--|
| 四级闸门状态机+主闸门 | plugins/dsh-extra-plan/index.js | 修改最频繁（route/purpose/clarified/approved/channelBroken）；7 个闸门关键词的唯一值源是 YAML 的 config.gateWords，词表作为显式参数贯穿全部 helper/状态机/闸门（apply 第一步 createGateRuntime 严格校验并在当前 agent scope 注册 7 个 extra_plan_* 变量） |
...
## P2-4 B1/B2 边界补充
- exploreBudget 默认链固定为 YAML 模板叶值 → 构建期生成模块 → runtime fallback；`preset-defaults.generated.js` 带 generated/do not edit 头，descriptor 不拥有默认值，`index.js` 不在运行时解析 YAML。
