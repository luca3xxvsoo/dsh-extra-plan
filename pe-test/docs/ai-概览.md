...
兼容：**仅 dsh 0.1.7-rc.1 / 0.1.7-rc.2**（peerDependencies 精确钉 `'0.1.7-rc.1 || 0.1.7-rc.2'`；不再支持 0.1.2-rc.1~0.1.5-rc.2 同码兼容）；qqbot 0.5.0 版（自愈/建链由精简版 dsh-qqbot-user-questions 承担；两行已按 0.1.7 新包族静态重写，**待有 QQBOT 环境再测试**）；Linux/macOS 逻辑层已验证（pe-test 写拦截 68 用例），运行时仅 Windows 实测。
...
| 闸门关键词共享契约（单一来源） | lib/gate-words.js（GATE_WORD_FIELDS/GATE_WORDS_GROUP_DEFINITION/GATE_WORD_MIGRATION_DEFINITIONS/validateGateWords/createGateRuntime）；值写在 assets/presets/extra-plan/agent.cordis.yml 的 config.gateWords | 纯模块：不含任何出厂词值、不读文件/环境变量、无无参默认词表；校验失败一律 'extra-plan: config.gateWords' 前缀；被 index.js（运行时）与 lib/preset-sync.js（升级迁移）共享；10 项 UI settings 之外并列 7 项 migration-only 字段（不进设置页 metadata、不进 preset-defaults.generated.js、不新增构建步骤） |
...
| 设置页宿主半段 | lib/settings.js（createApiHandler + Config 8 字段 volatile） | 8 项 UI 设置走 settings 行 Config（官方 configForms）；2 项宿主行（webFetch/toolPresentationMode）经专用 PUT → ctx.configEditor.edit 整体重述声明行 plugins；仅本机环回，失败口径 404（行定位）/500（edit/reconcile） |
| 设置页前端 UI | lib/client.js | 打包器（__ModuleLoader__）格式；注册面 = Plugins 页 `plugins.item`（configForms.whileServed 包裹），inject ['slots','locale','configForms']；8 项消费 ownerProps.form（state/mutate），2 项自绘控件走 PRO_CONFIG_URL PUT |
| 预设自愈核对 | lib/preset-sync.js（apply 经 ctx.inject(['configEditor'])；写盘只走 configEditor.edit） | 启动时按「资产 hash + 声明行覆盖资产行 id 集合」判定；两条保留链：一致 → idle（不写盘）；不一致 → 8 项写 settings 行、2 项写声明行 plugins 子行、整组迁移合法旧 gateWords（缺失/非法/歧义整组缺席，禁止部分迁移）；厂商模板 gateWords 在 hash/idle 判定前严格校验；manifest format=2 + settingsMigration 10 项 + gateWordsMigration 7 项（只记状态不记词值），状态目录 $DSH_HOME/.agent-presets/extra-plan/ |
...
| 预设设置解析与行定位 | lib/preset-settings.js（loadYaml/captureSettings/captureRowSettings/restatePluginsRow/patchYamlScalar/findPluginsRow 等） | 十项设置 descriptor，每项双定位元数据：`sourceLocator`（源模板/旧分发副本，id extra-plan/tool-web/tool-presentation + config 路径）与 `rowLocator`（新载体：8 项 settings 行 dsh-extra-plan-settings、2 项声明行 plugins 子行）；`group` 标记 extra-plan(8)/host-rows(2)；已无 pluginId/path 顶层字段；js-yaml 双路回退 |
...
- postinstall 语义（0.1.7 起）：scripts/distribute-preset.mjs 只初始化插件自有状态目录 + 空 manifest（`distHash: null`），**不再分发预设本体**（预设本体随 `dsh.bundle.patch` 数组的 preset-patch.generated.yml 装载）
...
## 闸门关键词单源与升级迁移（v0.3.0）
- **唯一值源**：`config.gateWords`（7 字段集中排列，agent.cordis.yml）；JS 侧 `lib/gate-words.js` 只有字段规格/严格校验/运行时派生与迁移 locator，**无任何出厂词值、无默认词表**；persona 的 `prefix`/`text` 双键只引用 7 个 `{{extra_plan_*}}` 变量。
...
- **两条保留链**：同 hash 普通重启 → `idle`（三核心文件逐字节不变，现场非法也不自愈，随后 runtime 抛错阻止使用）；hash 变化版本升级 → 新模板优先 + 只迁 10 项设置与 7 个词值（persona/注释/其它 config 均取新版）。
- **审计**：manifest 保持 `format: 2`；`settingsMigration` 仍 10 项；新增并列字段 `gateWordsMigration: { format: 1, sourceDistHash, source, results }`，results 恰 7 项状态字符串，不记录用户实际词值。`gateWordsMigration` 是项目自有字段，不是宿主合同。
...
- 生成器与 prepack 先校验后替换；坏模板保持 last-known-good，preset-sync 在目标目录写入前失败，postinstall/startup 仍由既有外壳吞错不阻断。
