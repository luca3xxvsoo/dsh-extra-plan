...
| 设置页前端 UI | lib/client.js | 打包器（__ModuleLoader__）格式；注册面 = Plugins 页 `plugins.item`（configForms.whileServed 包裹），inject ['slots','locale','configForms']；单卡双区块（通用设置 3+2 项 / pro规划 5 项），全卡唯一保存按钮：先 PUT 2 项宿主行（PRO_CONFIG_URL）再走官方 configForms mutate 8 项，部分失败自动回滚 2 项并提示；入口 = DSH web 界面 → 左侧边栏「插件」页 → 官方分组 →「按需规划模式配置」 |
