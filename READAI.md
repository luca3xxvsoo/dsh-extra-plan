...
兼容：**仅 dsh 0.1.7-rc.1 / 0.1.7-rc.2**（package.json peerDependencies 精确钉 `'0.1.7-rc.1 || 0.1.7-rc.2'`——两版逐字列举、无 ^/~ 范围符；不再支持 0.1.2-rc.1~0.1.5-rc.2 同码兼容；rc.2 单侧复核（rc.1 只读现场已不存在）见 ai-宿主耦合台账.md §⑦-2）；qqbot 0.5.0 版 + 精简版 dsh-qqbot-user-questions（仅自愈+mklink，选装；qqbot 侧两行按新包族静态重写，**【待有 QQBOT 环境再测试，本机不做验证】**）。A=anchoredBootstrap、C=creativeMode、M=toolPresentationMode（native/ptc/both），F=尚无 tool/call、L=首个 tool/call 后。
